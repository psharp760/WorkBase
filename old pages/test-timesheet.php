...
function getMonthYear($month, $year){
   $monthInString = array("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December");
  		echo "<div class='monthly-btns'></div>";
        echo "<button id='previous' type='submit' class='previousButton' name='previous' onclick='moveTimesheet(this.id)'></button>";
        echo "<div class='month-box' id='monthYear'>".$monthInString[$month-1]." ".$year."</div>";
        echo "<button id='next' type='submit' class='nextButton' name='next' onclick='moveTimesheet(this.id)' ></button>";
        echo "</div></form>";
   //echo $display;
}
function IndividualTimesheet($month, $year){
   ...
   <header>
      ...
      <div class="nav-container">
         <ul id="nav">
            ...
            <li><img src="images/timesheet-icon.png" id="icon-nav"><a href="test-timesheet.php" id="current">Timesheet</a></li>
...
<main> 
<form action="test-timesheet.php" method="post">
...
//$q = intval($_GET['q']);
//if($option>0){ 
   IndividualTimesheet($_SESSION['month'], $_SESSION['year']);
//}
/*else{
