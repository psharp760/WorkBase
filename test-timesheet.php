...
$startDay = date('w', strtotime('Y-m-01'));
...
$calendorText = $monthInString[$month-1]." ".$year;

function getCaledarMonth($y, $m, $date){
   $curYear = $y;
   $curMonth = $m;
...
function getMonthYear($month, $year){
   ...
   $display .= "<div class='monthly-btns'></div>"
            . "<button id='previous' type='submit' class='previousButton' name='previous' onchange='moveTimesheet'></button>"
            . "<div class='month-box' id='monthYear'>".$monthInString[$month-1]." ".$year."</div>"
            . "<button id='next' type='submit' class='nextButton' name='next' onchange='moveTimesheet'></button>"
            . "</div></form>";
   echo $display;
}
...
//$sql = "SELECT user_id, first_name, last_name FROM users ORDER BY user_id";
//$result = mysqli_query($con, $sql);
//$resultCount = mysqli_num_rows($result);

//$display = "";

// Timesheet contents (all or individual timesheet)
if($q>0){ // Individual timesheet
   ...
   for($i=0; $i<$endDayCount+1; $i++){
      $display .= "<tr><td id='sub-left-height'>".$month."/".($i+1)."</td> 
            <td id='sub-left-height'>".$dayInString[($startDay+$i+2)%7]."</td>";
   ...
   $display .= "</table></div>";
   echo $display; 
} // end of Individual timesheet contents.
}else{
   $display .= "<div class='table-contents'> <table> <thead> <tr> 
         <th id='user-head'>Employees</th>";
